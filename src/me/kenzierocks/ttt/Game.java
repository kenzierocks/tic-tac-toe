...
public class Game {
	...
	private boolean allFilled() {
		...
		for (int i = 0; i < this.data.length; i++) {
			...
			for (int j = 0; j < row.length; j++) {
				char mark = row[j];
				anyOff |= mark != Player.X.character && mark != Player.O.character;
			}
		}
		return !anyOff;
	}
	...
	private char getOr0(Direction dir, int times, int x, int y) {
		x += dir.dx * times;
		y += dir.dy * times;
		if (x >= this.data.length || x < 0 || y >= this.data[0].length || y < 0) {
			return '\0';
		}
		return get(x, y);
	}
	...
	private boolean winning(char p, int x, int y) {
		...
		return directions.anyMatch(
				dir -> IntStream.rangeClosed(-2, 2).map(t -> getOr0(dir, t, x, y)).map(c -> c == p ? 1 : 0).sum() == 3);
	}
