...
public class Controller {
	...
	public void restart() {
		...
		updateScores();
		updateCurrentPlayer();
		gameplayPane.getChildren().stream().map(Label.class::cast).forEach(label -> {
			label.setText("[  ]");
				...
				if (win != WinState.NEUTRAL) {
					Platform.runLater(() -> {
						...
						if (win == WinState.WIN) {
							...
							if (currentPlayer == 'X') {
								playerData.setXScore(playerData.getXScore() + 1);
							} else if (currentPlayer == 'O') {
								playerData.setOScore(playerData.getOScore() + 1);
							} else {
								throw new IllegalStateException("Player who now? " + currentPlayer);
							}
	...
	public void initialize() {
		gameplayPane.getChildren().stream().map(Label.class::cast).forEach(Util::attachColorChange);
